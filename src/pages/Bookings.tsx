...
export default function Bookings() {
  ...
  const nights = checkIn && checkOut ? Math.max((new Date(checkOut).getTime() - new Date(checkIn).getTime()) / (1000*60*60*24), 0) : 0;
    ...
    <div className="max-w-lg mx-auto p-6 bg-white rounded shadow">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input className="w-full border rounded p-2" type="text" {...register('roomId')} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input className="w-full border rounded p-2" type="date" {...register('checkInDate')} />
            {errors.checkInDate && <p className="text-red-600 text-sm">{errors.checkInDate.message}</p>}
          </div>
          <div>
            <label className="block mb-1">Check-out Date</label>
            <input className="w-full border rounded p-2" type="date" {...register('checkOutDate')} />
            {errors.checkOutDate && <p className="text-red-600 text-sm">{errors.checkOutDate.message}</p>}
          </div>
        </div>
        <div>
          <label className="block mb-1">Special Requests</label>
          <textarea className="w-full border rounded p-2" {...register('specialRequests')} />
        </div>
