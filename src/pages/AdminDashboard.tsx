...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/bookings', { headers }).then(r => r.json()).then(json => setBookings(json.data || json));
    fetch('/api/rooms').then(r => r.json()).then(json => setRooms(json.data || json));
  }, [token]);
          ...
          <ul className="space-y-2">
            {bookings.map(b => (
              <li key={b.id} className="flex justify-between items-center border rounded p-2">
                ...
                <button className="bg-green-600 text-white px-3 py-1 rounded" onClick={() => confirmBooking(b.id)}>Confirm</button>
              </li>
            ))}
          </ul>
        </div>
